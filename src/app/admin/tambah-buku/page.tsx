...
export default function TambahBukuPage() {
  ...
  // Helper: convert OpenLibrary subjects ke options react-select
  ...
  async function fetchByISBN(isbn: string) {
    ...
    try {
      ...
      if (details) {
        ...
        setPengarang(
          details.authors?.map((a: any) => a.name).join(", ") || ""
        );
        setPenerbit(
          details.publishers?.map((p: any) => p.name).join(", ") || ""
        );
        setTahun(details.publish_date || "");
        const opts = subjectsToOptions(details.subjects?.slice(0, 10) || []);
        setKategori(opts);
      } else {
        alert("Data buku tidak ditemukan di OpenLibrary");
      }
    } catch (err) {
      alert("Gagal mengambil data buku: " + err);
    }
    setLoading(false);
  }
  ...
  function handleScanResult(code: string) {
    ...
    // Validasi ISBN/EAN13: hanya angka, 10 sampai 13 digit
    if (/^\d{10,13}$/.test(code)) {
      setIdBuku(code);
      fetchByISBN(code);
    } else {
      // Kode lain selain angka 10-13 digit, simpan saja ID buku tanpa fetch
      setIdBuku(code);
    }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    alert("Data buku berhasil disiapkan untuk dikirim ke backend.");
  }
